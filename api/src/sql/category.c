/**
 * @file category.c
 * @brief SQLite data-access implementation for the Category table.
 */
...
#define QUERY_PUT_TMP                                                          \
  ...
  "SET name = ?, color = ? "                                                   \
  "WHERE name = ?;"
#define QUERY_DELETE_TMP "DELETE FROM Category WHERE name = ?;"
...
int category_exists(char *name) {
  ...
  while (query_rc != SQLITE_DONE) {
    if (sqlite3_column_type(stmt, 0) == SQLITE_INTEGER) {
      categories_count = sqlite3_column_int(stmt, 0);
      printf("COUNT:\t%d\n", categories_count);
    }
...
int get_categories(size_t len, struct category **arr, const struct mg_str *q,
  ...
  while (query_rc == SQLITE_ROW && count < len) {
    struct category *u = NULL;
    u = malloc(sizeof(struct category));
    ...
    int category_rc = category_map(u, stmt, 0, 1);
    if (category_rc != 0) {
      free(u);
...
int get_category(struct category *category, char *name) {
  ...
  while (query_rc == SQLITE_ROW) {
    ...
    int category_rc = category_map(category, stmt, 0, 1);
    if (category_rc != 0) {
      free(category);
...
int add_category(struct category *category) {
  ...
  sqlite3_bind_text(stmt, 2, category->color, -1, SQLITE_STATIC);
