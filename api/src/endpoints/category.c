/**
 * @file category.c
 * @brief Category endpoint handler implementations (list, single resource).
 */
...
void send_categories_res(struct mg_connection *c, struct mg_http_message *msg,
  ...
  if (mg_match(msg->method, mg_str("GET"), NULL)) {
    printf(TERMINAL_ENDPOINT_MESSAGE("=== GET CATEGORY LIST ==="));
    ...
    reply->json = NULL;
    reply->total = reply->count = get_categories_len(&q);
    reply->total_pages = 0;
    ...
    if (reply->count > 0) {
      categories = malloc(reply->count * sizeof(struct category *));
      query_code = get_categories(reply->count, categories, &q, &sort,
    ...
    reply->data = categories_to_json(categories, reply->count);
    list_reply_to_json(reply);

    SUCCESS_REPLY_200(reply->json);
    printf(TERMINAL_SUCCESS_MESSAGE("=== CATEGORIES SUCCESSFULLY SENT ==="));

    if (reply->count > 0) {
      free_categories(categories, reply->count);
      free(reply->data);
    }
    free(reply->json);
    free(reply);
    ...
    if (exists != 0) {
      ERROR_REPLY_400(CATEGORY_EXISTS_MESSAGE);
      free(name);
      return;
    };
    ...
    if (category_init_rc != 0) {
      ERROR_REPLY_500;
      fprintf(stderr, TERMINAL_ERROR_MESSAGE("CATEGORY IS NULL"));
      free(name);

      return;
    }

    category_hydrate(msg, category);

    // Store in DB
    query_code = add_category(category);
    if (query_code != 0) {
      fprintf(stderr, TERMINAL_ERROR_MESSAGE("ERROR RETRIEVING CATEGORIES"));
      HANDLE_QUERY_CODE;
      free(name);
      ...
      free(result);
      printf(TERMINAL_SUCCESS_MESSAGE("=== CATEGORY SUCCESSFULLY ADDED ==="));
    }
...
void send_category_res(struct mg_connection *c, struct mg_http_message *msg,
  ...
  if (mg_match(msg->method, mg_str("GET"), NULL)) {
    printf(TERMINAL_ENDPOINT_MESSAGE("=== GET CATEGORY ==="));
    ...
    if (query_code != 0) {
      fprintf(stderr, TERMINAL_ERROR_MESSAGE("ERROR RETRIEVING CATEGORY"));
      HANDLE_QUERY_CODE;
      ...
      printf(TERMINAL_SUCCESS_MESSAGE("=== CATEGORY SUCCESSFULLY SENT ==="));
    }
    ...
    // Store in DB
    query_code = edit_category(category, name);
    if (query_code != 0) {
      fprintf(stderr, TERMINAL_ERROR_MESSAGE("ERROR RETRIEVING CATEGORIES"));
      HANDLE_QUERY_CODE;
      ...
      SUCCESS_REPLY_200(result);
      free(result);
      printf(TERMINAL_SUCCESS_MESSAGE("=== CATEGORY SUCCESSFULLY EDITED ==="));
    }
    ...
    int delete_rc = delete_category(name);
    if (delete_rc != 0) {
      ERROR_REPLY_500;
      fprintf(stderr, TERMINAL_ERROR_MESSAGE("COULDN'T DELETE CATEGORY"));
    }

    printf(TERMINAL_SUCCESS_MESSAGE("=== CATEGORY SUCCESSFULLY DELETE ==="));
    SUCCESS_REPLY_200_MSG("Category successfully deleted");
  } else {
    ERROR_REPLY_405;
  }
}
