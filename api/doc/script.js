...
window.addEventListener("load", async () => {
  ...
  const endpointsContainer =
    apiContainer.querySelector("#endpoints");
  ...
  collapseAllButton = document.querySelector(
    "button#collapse-all",
  );
...
const displayAuthForm = () => {
  ...
  if (!isUserLogged) {
    ...
    const logoutButtonNode =
      authContainer.querySelector("button");
...
const getEndpointNode = (endpoint) => {
  ...
  if (uriParameters !== undefined && uriParameters.length > 0) {
    ...
    uriParametersContainer.appendChild(
      uriParametersContainerTitle,
    );
  ...
  if (
    ...
    const queryParametersContainer =
